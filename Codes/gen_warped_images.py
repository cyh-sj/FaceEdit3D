...
def gen_interp_video(G, outdir: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), ifstyle=False, **video_kwargs):
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        initail_ldks_path = "source_3d_ldms.txt"
        target_ldks_path = "target_3d_ldms.txt"
        ...
        for angle in angles:
            ...
            PIL.Image.fromarray(img, 'RGB').save(f'{outdir}/seed{seed:04d}_direction{angle:.2f}.png')
            PIL.Image.fromarray(ori_img, 'RGB').save(f'{outdir}/origin{seed:04d}_direction{angle:.2f}.png')
